...
static char buf[256];
static struct semaphore Sem; /* semaphore associe au buffer buf */
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,3,0)
    if (down_interruptible(&Sem)) return(0);
#endif
    len = sprintf(buf, "%9li %9li %9li\n", j0, j1, j1-j0);
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,3,0)
    *start = buf;
#else
    _copy_to_user(bufu, buf,len);
    up(&Sem);
#endif
    return(len);
}
...
#else
   sema_init(&Sem, 1); /* initialisation du semaphore */
